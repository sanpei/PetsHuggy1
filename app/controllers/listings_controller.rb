class ListingsController < ApplicationController
  ...
  def create
    ...
    if @listing.save
      redirect_to manage_listing_basics_path(@listing), notice: "リスティングを作成と保存をしました"
    else
      redirect_to new_listing_path(@listing), notice: "リスティングを作成と保存できませんでした"
    end
  end
  ...
  def listing_params
    params.require(:listing).permit(:home_type, :pet_type, :breeding_years, :pet_size, :price_pernight, :address,
      :listing_content, :listing_title, :active)
  end
